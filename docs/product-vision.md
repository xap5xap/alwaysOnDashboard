...
- **Google Calendar integration spec (second per-integration spec, first REST service):** [specs/integration-calendar.md](specs/integration-calendar.md) locks how Google Calendar plugs into the shipped seam as a registration-only add, the second proof that the [AOD-55](https://linear.app/thexap/issue/AOD-55) operation seam generalizes beyond GraphQL. Calendar OAuth uses the `calendar.readonly` scope (verified to authorize both `events.list` and `calendarList.list`, so the calendar picker works on the shipped single scope), PKCE, and `access_type=offline`/`prompt=consent` for a roughly 1h access token plus a preserved refresh token, which closes [AOD-9](https://linear.app/thexap/issue/AOD-9) §11's Google item and confirms §4. It fixes the Next Event and Today's Agenda `events.list` requests and normalized payloads (`NextEventData`/`AgendaData` over a shared `CalendarEvent`, with the agenda's "today" boundary applied in the renderer), the `calendarId` config with the `google_calendars` option source (the first real consumer of `providerBackedSource`), and refines the [AOD-55](https://linear.app/thexap/issue/AOD-55) seam once for REST (an optional `buildQuery` and registry path-token substitution, with `buildBody` made optional; backward compatible for Linear, Weather, and the stub). TTLs are justified against Google's quotas, and the Google 403 `usageLimits` rate-limit mapping is named as a one-line seam. Google Calendar API verified 2026-06-26. (2026-06-26) — [AOD-32](https://linear.app/thexap/issue/AOD-32)
