...
>
> Amended 2026-06-26 ([AOD-31](https://linear.app/thexap/issue/AOD-31)): Linear's OAuth specifics were verified at wiring (see [`integration-linear.md`](integration-linear.md)): the access token expires in 24h and Linear issues a refresh token, so Linear is a refreshing `oauth2` service. This resolves the §11 Linear item and updates the §4 note; the broker's conditional-refresh handling is unchanged.
...
- **`oauth2`.** Linear is GraphQL and uses an authorization-code flow. Verified at wiring ([AOD-31](https://linear.app/thexap/issue/AOD-31), 2026-06-26): the access token expires in 24h and Linear issues a refresh token, so Linear is refreshed like Google. The model still treats refresh as conditional on the token response (an expiry plus a refresh token present), not assumed, so it stays correct even if a provider returns a longer-lived token. Google Calendar issues a short-lived access token (about one hour) plus a refresh token, and needs `access_type=offline` with `prompt=consent` to reliably return that refresh token. The flow is the same code path; the registry entry carries the per-provider parameters.
...
- **Per-provider OAuth specifics to verify when wiring each service:** Linear's token lifetime and whether it issues refresh tokens (resolved by [AOD-31](https://linear.app/thexap/issue/AOD-31): a 24h access token and a refresh token, verified 2026-06-26); Google's `access_type=offline` / `prompt=consent` requirements and refresh-token rotation behavior; each provider's revoke endpoint.
