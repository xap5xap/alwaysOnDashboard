...
- The four credential classes the v1 service set needs: OAuth, API key, Admin key, and no-auth.
...
| `api_key` | Weather | user-supplied API key | No | the API key | No |
...
- **`api_key`.** Weather is the example. The user pastes a provider API key once; it is stored encrypted and attached server-side on every proxied call. (Alternative considered: a single platform-owned weather key shared across users, where the user only supplies a location and there is no per-user credential. The registry can model either; the per-user `api_key` class is specified here because it is the general case the broker must support.)
...
| `auth_class` | `text` | `oauth2` / `api_key` / `admin_key` |
| `status` | `text` | `connected` / `reauth_required` / `error` / `disconnected` |
| `scopes` | `text[]` | granted scopes (oauth2) |
| `access_secret_id` | `uuid` | reference into `vault.secrets` for the access token or API/Admin key |
...
| `credentials-store` | api_key, admin_key | Accept a user-supplied key over TLS, optionally test it with one provider call, store it in Vault, write the connection row. |
...
Adding a service is a registry entry plus, for oauth2, its client secret in Edge env. The proxy only ever calls `api_base` + an allow-listed `widgets[...]` path, which is what makes goal 5 (no open relay) hold.
...
### 7.2 API-key and Admin-key services (Weather, Anthropic usage)

No browser and no code exchange. The app collects the key in a form and POSTs it once to `credentials-store` over TLS. The function may make a single validation call to the provider, then stores the key in Vault and writes the `connections` row (`status=connected`, no `expires_at`, no refresh reference). The key is never returned to the device afterward; the Settings UI shows only a masked hint and the `account_label`.
...
For `oauth2` connections that return an expiry, `expires_at` is the access-token deadline. Connections with no expiry (long-lived Linear tokens, API keys, Admin keys) leave `expires_at` null and are never selected for refresh.
...
4. It reads the access token, API key, or Admin key from Vault.
5. It maps `service` + `widget` to an allow-listed endpoint in the registry and calls the provider, attaching the secret in the header style the registry declares (Bearer for Linear and Google, the provider's API-key header for Weather, the Admin key for the two Anthropic Admin endpoints).
...
4. The function deletes the access and refresh Vault secrets and retires the connection row (see the policy note below).
...
- **Weather credential model.** Per-user API key (specified here) versus a single platform key with user-supplied location. Decide alongside the v1 widget set in [AOD-4](https://linear.app/thexap/issue/AOD-4).
