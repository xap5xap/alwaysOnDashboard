...
>
> **FORWARD-NOTE 2026-07-22 (AOD-197 S4, the layout-foundation rework).** The authored board is now the **responsive per-orientation grid** ([`design-layout-foundation.md`](design-layout-foundation.md), LOCKED 2026-07-22): **6 columns landscape / 4 portrait**, free placement **with gaps** ("place, don't pack" — a move relocates only the dragged card, no neighbour auto-pack), and on the handheld the cells **fit to width** via an additive `cellPx` scale applied over the nominal **`UNIT_PX = 96`** grid (the card internals and this wall never see a changed unit). **The wall is unchanged:** it requests the **landscape** layout and scales it **fit-to-bounds** through `wallFitScale` (this doc's REVISED §3/§4 policy) — landscape-locked, zero-interaction, **byte-identical**. `KioskWall.tsx` / `viewport.ts` show zero diff across the rework.
>
